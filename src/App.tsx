...
const InteractiveVideoGuide: React.FC = () => {
  ...
  // ---- Auto-fit preview sizing (keeps 16:9) ----
  const previewHostRef = useRef<HTMLDivElement | null>(null);
  const [hostWidth, setHostWidth] = useState<number>(640);

  useEffect(() => {
    const el = previewHostRef.current;
    if (!el) return;

    // Measure the container and react to resizes
    const ro = new ResizeObserver(entries => {
      const w = entries[0].contentRect.width;
      // Subtract a little for the card padding so the box doesn’t overflow
      setHostWidth(Math.max(320, w - 32));
    });
    ro.observe(el);

    // Initial measure (in case RO fires late)
    setHostWidth(Math.max(320, el.clientWidth - 32));

    return () => ro.disconnect();
  }, []);

  // Max visual width for the preview (bump if you want even larger)
  const MAX_W = 1280;
  const aspect = 16 / 9;
  const vw = Math.min(hostWidth, MAX_W);
  ...
  // Scale crop/mask from 1920×1080 to the current visual vw/vh
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 text-white p-6">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Give more width to the preview column on large screens */}
        <div className="grid gap-6 lg:grid-cols-[1.05fr_1.45fr]">
            ...
            {activeTab === 'basics' && (
              <div className="space-y-3">
                <div>
                  ...
                  {videoFile && <p className="text-xs text-green-400 mt-1">✓ Loaded: {videoFile.name}</p>}
                </div>
                ...
                <div className="bg-purple-900/30 border border-purple-500/50 rounded-lg p-3">
                  <p className="text-sm font-bold">
                    Duration: <span className="text-lg text-purple-300">{duration}s</span>
                  </p>
                </div>
              </div>
            )}
            ...
            {activeTab === 'crop' && (
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <input
                    type="number"
                    value={formData.crop_top}
                    onChange={(e) => handleChange('crop_top', e.target.value)}
                    className="w-full bg-slate-900/80 border border-slate-600 rounded-lg px-3 py-2 focus:border-purple-500 focus:outline-none"
                  />
                </div>
                <div>
                  ...
                  <input
                    type="number"
                    value={formData.crop_bottom}
                    onChange={(e) => handleChange('crop_bottom', e.target.value)}
                    className="w-full bg-slate-900/80 border border-slate-600 rounded-lg px-3 py-2 focus:border-purple-500 focus:outline-none"
                  />
                </div>
                <div>
                  ...
                  <input
                    type="number"
                    value={formData.crop_left}
                    onChange={(e) => handleChange('crop_left', e.target.value)}
                    className="w-full bg-slate-900/80 border border-slate-600 rounded-lg px-3 py-2 focus:border-purple-500 focus:outline-none"
                  />
                </div>
                <div>
                  ...
                  <input
                    type="number"
                    value={formData.crop_right}
                    onChange={(e) => handleChange('crop_right', e.target.value)}
                    className="w-full bg-slate-900/80 border border-slate-600 rounded-lg px-3 py-2 focus:border-purple-500 focus:outline-none"
                  />
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      type="number"
                      value={String(formData.mask_x)}
                      onChange={(e) => handleChange('mask_x', e.target.value)}
                      className="w-full bg-slate-900/80 border border-slate-600 rounded-lg px-3 py-2 focus:border-purple-500 focus:outline-none"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      type="number"
                      value={String(formData.mask_y)}
                      onChange={(e) => handleChange('mask_y', e.target.value)}
                      className="w-full bg-slate-900/80 border border-slate-600 rounded-lg px-3 py-2 focus:border-purple-500 focus:outline-none"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      type="number"
                      value={String(formData.mask_w)}
                      onChange={(e) => handleChange('mask_w', e.target.value)}
                      className="w-full bg-slate-900/80 border border-slate-600 rounded-lg px-3 py-2 focus:border-purple-500 focus:outline-none"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      type="number"
                      value={String(formData.mask_h)}
                      onChange={(e) => handleChange('mask_h', e.target.value)}
                      className="w-full bg-slate-900/80 border border-slate-600 rounded-lg px-3 py-2 focus:border-purple-500 focus:outline-none"
                    />
                  </div>
                </div>
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => handleChange('FinalName', e.target.value)}
                    className="w-full bg-slate-900/80 border border-slate-600 rounded-lg px-3 py-2 focus:border-purple-500 focus:outline-none"
                  />
                </div>
              </div>
          ...
          <div className="space-y-4">
            <div className="bg-slate-800/70 backdrop-blur rounded-2xl p-6 border border-slate-600">
              ...
              {/* Attach ref here so the preview auto-fits this card */}
              <div ref={previewHostRef} className="bg-slate-900/80 rounded-lg p-4 mb-4">
                <div
                  style={{
                    width: vw,
                    height: vh,
                  ...
                  {videoUrl ? (
                    ...
                    >
                      ...
                      <span style={{ fontSize: '9px', color: '#64748b' }}>
                        Go to Basics tab → Upload video for preview
                      </span>
                    </div>
                  )}
                      ...
                      {cT > 0 && (
                        <div
                          style={{
                            ...
                            height: cT,
                            background:
                              'repeating-linear-gradient(45deg, rgba(239,68,68,0.5), rgba(239,68,68,0.5) 10px, rgba(220,38,38,0.5) 10px, rgba(220,38,38,0.5) 20px)',
